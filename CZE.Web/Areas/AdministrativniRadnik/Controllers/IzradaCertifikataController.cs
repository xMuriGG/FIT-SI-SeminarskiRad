...
namespace CZE.Web.Areas.AdministrativniRadnik.Controllers
{
    public class IzradaCertifikataController : Controller
    {
        ...
        public ActionResult Create(CertifikatEditVM model)
        {
            ...
            int logiraniKorisnikId = Autentifikacija.GetLogiraniKorisnikId(HttpContext); ;
            if (db.Zaposlenici.Any(x => x.ZaposlenikID == logiraniKorisnikId))
            {
                try
                {
                    model.ZaposlenikID = logiraniKorisnikId;
            ...
            {
                TempData["InfoMessage"] = new InfoMessage("Logirani korisnik mora biti zaposlenik da bih odobrio certifikat.", InfoMessageType.warning);
            }
